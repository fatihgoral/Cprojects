...
#define GUC 7
#define CEVIKLIK 8
#define DAYANIKLILIK 9
#define KARIZMA 10
#define TOPLAYICILIK 11

// Karakter özellikleri dizisi
int ozellikler[12] = {10, 1, 0, 100, 100, 100, 100, 3, 3, 3, 3, 3};
...
int main() {
    ...
    while (1) {
        ...
        if (ozellikler[TOKLUK] <= 0 || ozellikler[UYKU] <= 0 || ozellikler[HIJYEN] <= 0) {
            int canKaybi = 0;
...
void han(int ozellikler[]) {
    ...
    printf("Guncel Durum -> Tokluk: %d, Hijyen: %d, Altin: %d\n", ozellikler[TOKLUK], ozellikler[HIJYEN], ozellikler[ALTIN]);
}
